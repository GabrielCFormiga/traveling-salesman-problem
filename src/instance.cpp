...
void Instance::read(){
	...
	std::ifstream inTSP(m_name, std::ios::in);
	...
	inTSP >> m_dimension; // valor depois do std::string "DIMENSION:" é a dimensão do problema
	...
	xCoord = std::shared_ptr<double[]>(new double [ m_dimension ]); //coord x
	yCoord = std::shared_ptr<double[]>(new double [ m_dimension ]); //coord y
	...
	if ( typeProblem == "EXPLICIT" ) {
		...
		while ( file.compare("NODE_COORD_SECTION") != 0 ) { //enquanto nao achar o std::string "NODE_COORD_SECTION" faça
			inTSP >> file;
		}
		// ler coordenadas
		...
		double *latitude = new double [ m_dimension ];
		double *longitude = new double [ m_dimension ];
...
double Instance::CalcDistEuc ( double *X, double *Y, int I, int J ){
	return
			sqrt ( pow ( X[I] - X[J], 2 ) + pow ( Y[I] - Y[J], 2 ) );
}

double Instance::CalcDistAtt ( double *X, double *Y, int I, int J )
{
...
void Instance::CalcLatLong ( double *X, double *Y, int n, double *latit, double* longit )
{
...
double Instance::CalcDistGeo ( double *latit, double *longit, int I, int J )
{
...
{ // Get the name of instance
	...
	std::string::size_type loc = m_name.find_last_of(".", m_name.size() );
	std::string::size_type loc2 = m_name.find_last_of("/", m_name.size() );

	std::string instance;

	if (loc != std::string::npos) {
		instance.append(m_name, loc2+1, loc-loc2-1 );
	} else {
		instance.append(m_name, loc2+1, instance.size() );
	}
